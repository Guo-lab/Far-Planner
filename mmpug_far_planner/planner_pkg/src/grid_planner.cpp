...
inline int GridPlanner::computeKey(int x, int y) { return (y * y_size + x); }
 ...
 *      dx + dy - min(dx, dy)
 */
float GridPlanner::estimateOctileDistance(int curr_x, int curr_y, int goal_x,
                                          int goal_y) {
 ...
 *     sqrt(dx^2 + dy^2)
 */
float GridPlanner::estimateEuclideanDistance(int curr_x, int curr_y, int goal_x,
                                             int goal_y) {
  int delta_x = abs(curr_x - goal_x);
  int delta_y = abs(curr_y - goal_y);
 ...
 */
void GridPlanner::updateMap(const nav_msgs::OccupancyGrid& grid,
 ...
 */
int GridPlanner::naivePlanner(const geometry_msgs::Pose& robot_pose,
  ...
  while (!open_list.empty() &&
         closed_list.count(computeKey(goal_node.x, goal_node.y)) == 0) {
      ...
      for (int dir = 0; dir < 8; dir++) {
        ...
        if (expand) {
          if (newx >= 0 && newx < x_size && newy >= 0 && newy < y_size) {
            if (((int)map[getMapIndex(newx, newy)] >= 0) &&
                ((int)map[getMapIndex(newx, newy)] < obstacle_cost))  // if free
            {
              float g_s_dash = curr_node->g +
                               (int)map[getMapIndex(newx, newy)] +
                               sqrt(dX[dir] * dX[dir] + dY[dir] * dY[dir]);
              float h_s_dash = estimateEuclideanDistance(
                  newx, newy, goal_node.x, goal_node.y);
  ...
  if (pathFound) {
    ...
    while (backtrackNode->parent != NULL) {
      ...
      if (pathLength <= 1) {
        ...
        float prev_dir = atan2(last_wp_n1.position.y - last_wp_n2.position.y,
                               last_wp_n1.position.x - last_wp_n2.position.x);
        float curr_dir = atan2(wp.position.y - last_wp_n1.position.y,
                               wp.position.x - last_wp_n1.position.x);
        float dist = sqrt(std::pow(wp.position.x - last_wp_n1.position.x, 2) +
                          std::pow(wp.position.y - last_wp_n1.position.y, 2));
